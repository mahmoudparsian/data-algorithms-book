 ...
 * 
 */
 public class WikipediaKMeansUsingUtilVector {
     
    private static final Logger THE_LOGGER = Logger.getLogger(WikipediaKMeansUsingUtilVector.class);
   ...
   static Map<Integer, Vector> getNewCentroids(JavaPairRDD<Integer, Iterable<Vector>> pointsGroup) {
    ...
    static double getDistance(final List<Vector> centroids, final Map<Integer, Vector> newCentroids, final int K) {
       double distance = 0.0;
       for (int i = 0; i < K; i++) {
         distance += centroids.get(i).squaredDist(newCentroids.get(i));
       } 
       return distance;
    }
