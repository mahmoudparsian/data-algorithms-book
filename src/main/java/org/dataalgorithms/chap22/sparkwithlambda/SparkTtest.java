package org.dataalgorithms.chap22.sparkwithlambda;
...
public class SparkTtest {
   ...
   public static void main(String[] args) throws Exception {
      ...
      //                                                                   
      JavaPairRDD<String, String> pairs = biosets.mapToPair((String biosetRecord) -> {
      ...
      JavaPairRDD<String, Double> ttest =  
              grouped.mapValues((Iterable<String> biosets1) -> {
          Set<String> geneBiosets = new HashSet<String>();
          for (String biosetID : biosets1) {
              geneBiosets.add(biosetID);
          }
          // now we do need shared Map data structure to iterate over its items
          Map<String, Double> timetable1 = broadcastTimeTable.value();
          ...
          for (Map.Entry<String, Double> entry : timetable1.entrySet()) {
          ...
          double ttest1 = MathUtil.ttest(exist, notexist);
          return ttest1;
      });
